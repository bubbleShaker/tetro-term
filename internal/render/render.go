...
// Size はフレーム 1 つが占める桁数と行数を返す。枠を含む。
//
// 端末をこの大きさに合わせるのはドライバの仕事だが、**大きさを決めるのはここ**である。
// ブラウザ版は端末を固定サイズで作るので、その値を自前で持たせると
// セルの幅や盤面の大きさを変えたときに片方だけ古いまま残る。
// CLI 版が使わないのは、端末の大きさをこちらから決められないためにすぎない。
func Size() (cols, rows int) {
	return boardWidth + 2, game.Height + 2 // 左右の枠 / 上下の枠
}
