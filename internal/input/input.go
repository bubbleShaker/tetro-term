// Package input は押されたものを、ゲームへの入力に読み替える
// （→ CONTEXT.md「入力」「ドライバ」）。
//
// 読み替える相手は 2 つある。端末から届くバイト列（input.go）と、
// タッチボタンの id（touch.go）である。フレームが出口側の唯一の境界であるのと
// 対称に（→ docs/adr/0001）、**入口側の境界は「入力」ひとつ**であって、
// バイト列ではない。バイト列は端末という入力装置の都合にすぎず、
// 画面上のボタンにはその都合が無い。
//
// このファイルが受け持つバイト列の側が端末専用でないのは、xterm.js が本物の端末と
// まったく同じバイト列を送ってくるからである。M2 でブラウザ版を書くとき、
// キーの読み替えはここをそのまま使い回せた。
//
// **どちらの表もこのパッケージの中にある**ことが肝で、キーの綴りもボタンの id も
// 外へ出さない。片方だけを外に持たせると、割り当てを変えたときに直し忘れた側が
// 黙って動き続ける。
package input
